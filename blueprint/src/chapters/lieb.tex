...
Ando's convexity theorem by instantiating the operator power mean from
Chapter~5 at the pair of bounded operators $(R_B, L_A)$ acting on $H$,
and then reading off the resulting joint concavity or convexity of the
real-part inner product functional $x \mapsto \operatorname{re}\langle x,\, (\cdot)\, x\rangle$.
...
\begin{theorem}[Lieb concavity --- general form]
  ...
  \begin{proof}
    ...
    \leanok
    \uses{thm:power_mean_concave, prop:lmul_map_cfc}
    ...
    (Proposition~\ref{prop:lmul_map_cfc}), we have
...
\begin{theorem}[Ando convexity --- general form]
  ...
  \begin{proof}
    ...
    \leanok
    \uses{thm:power_mean_convex, prop:lmul_map_cfc}
    The argument is analogous to Theorem~\ref{thm:lieb_general}.
    The same CFC commutativity identities (Proposition~\ref{prop:lmul_map_cfc})
