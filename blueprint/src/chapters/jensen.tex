...
\begin{theorem}[Jensen's operator inequality]
  ...
  \begin{proof}
    \uses{lem:conj_sum_spectrum}
    We follow the unitary-averaging argument of Li and Wu.
    Write $m = n + 2$ and form the diagonal block element
    $D = \operatorname{diag}(a_0, \ldots, a_n, a_n) \in M_m(\mathcal{B})$
    (the last slot is a duplicate of $a_n$, introduced to make the matrix square).
    ...
    the remaining columns are filled in by a Gram--Schmidt procedure on the rows of
    $\mathcal{B}^m$.
