...
\begin{proposition}[CFC commutativity of $L$ and $R$]
  ...
  \begin{proof}
    ...
    For the $R_a$ branch, this uses the \texttt{opStar} isomorphism
    (Proposition~\ref{prop:opstar_cfc}) to transport CFC from $A$ to $A^{\mathrm{op}}$.
  \end{proof}
\end{proposition}
...
\begin{theorem}[Generalized perspective at commuting rpow pair]
  ...
  Let $\mathcal{B}$ be an ordered unital C$^*$-algebra, $f, g : \mathbb{R} \to \mathbb{R}$,
  and $L, R \in \mathcal{B}$ with $L$ strictly positive and $R \geq 0$.
  If $L$ and $R$ commute, then
  \[
    (f \triangle g)(L, R) \;=\; R^f \cdot L^g,
  \]
  where $R^f$ and $L^g$ denote CFC powers.
  \begin{proof}
