...
\begin{proposition}[H$^*$-algebra instance for matrices]
  \label{inst:hstar_matrix}
  \lean{FrobeniusMat.instHStarAlgebra}
  \leanok
  \uses{def:hstar_algebra}
  The algebra $M_n(\mathbb{C})$ of $n \times n$ complex matrices, equipped with the
  Frobenius inner product $\langle A, B \rangle = \operatorname{tr}(A^* B)$, is an
  H$^*$-algebra.
  \begin{proof}
    \proves{inst:hstar_matrix}
    \leanok
    Direct verification of the H$^*$-algebra axioms using the standard Frobenius
...
\begin{corollary}[Lieb extension theorem --- matrix form]
  ...
  \begin{proof}
    ...
    Apply the abstract Lieb extension theorem (Theorem~\ref{thm:lieb_extension})
    to $M_n(\mathbb{C})$ with the Frobenius inner product, and use
    the identification $\operatorname{re}\langle K,\, A^q K B^p \rangle =
    \operatorname{re}\operatorname{tr}(A^q K B^p K^*)$ together with the equivalences:
    $A$ is strictly positive iff $A$ is positive definite,
    and $B \geq 0$ iff $B$ is positive semidefinite.
  \end{proof}
\end{corollary}
...
\begin{corollary}[Lieb's concavity theorem --- matrix form]
  ...
  \begin{proof}
    ...
    \leanok
    \uses{thm:lieb}
    Apply the abstract Lieb concavity theorem (Theorem~\ref{thm:lieb})
    to $M_n(\mathbb{C})$ with the Frobenius inner product, and use
    the identification $\operatorname{re}\langle K,\, A^s K B^{1-s} \rangle =
    \operatorname{re}\operatorname{tr}(A^s K B^{1-s} K^*)$ together with the equivalences:
    $A$ is strictly positive iff $A$ is positive definite,
    and $B \geq 0$ iff $B$ is positive semidefinite.
  \end{proof}
\end{corollary}
...
\begin{corollary}[Ando's convexity theorem --- matrix form]
  ...
  \begin{proof}
    ...
    \leanok
    \uses{thm:ando}
    Apply the abstract Ando convexity theorem (Theorem~\ref{thm:ando})
    to $M_n(\mathbb{C})$ with the Frobenius inner product, and use
    the identification $\operatorname{re}\langle K,\, A^{-r} K B^q \rangle =
    \operatorname{re}\operatorname{tr}(A^{-r} K B^q K^*)$ together with the equivalences:
    $A$ is strictly positive iff $A$ is positive definite,
    and $B \geq 0$ iff $B$ is positive semidefinite.
  \end{proof}
\end{corollary}
