...
Helper lemmas for expressions of the form `∑ i, star (b i) * a i * b i`
...
lemma Finset.inf'_mem_of_forall_mem {ι : Type*} {s : Set ℝ}
    {t : Finset ι} (ht : t.Nonempty) {f : ι → ℝ} (hf : ∀ i ∈ t, f i ∈ s) :
    t.inf' ht f ∈ s := by
...
lemma Finset.sup'_mem_of_forall_mem {ι : Type*} {s : Set ℝ}
    {t : Finset ι} (ht : t.Nonempty) {f : ι → ℝ} (hf : ∀ i ∈ t, f i ∈ s) :
    t.sup' ht f ∈ s := by
...
lemma algebraMap_eq_sum_conj {A : Type*} [Semiring A] [StarMul A] [Algebra ℝ A]
...
lemma isSelfAdjoint_sum_conj {A : Type*} [NonUnitalSemiring A] [StarRing A]
...
lemma algebraMap_le_sum_conj {A : Type*} [Semiring A] [PartialOrder A]
...
lemma sum_conj_le_algebraMap {A : Type*} [Semiring A] [PartialOrder A]
    [StarRing A] [StarOrderedRing A] [Algebra ℝ A]
    {n : ℕ} {a b : Fin n → A} {r : ℝ}
    (hb : ∑ i, star (b i) * b i = 1)
...
theorem spectrum_sum_conj_subset {A : Type*} [Ring A] [StarRing A]
