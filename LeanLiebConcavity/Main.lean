...
theorem PerspectiveJointConvex
    ...
    have hT₂_star : star T₂ = (cfc g R) ^ (-1/2 : ℝ) * (a • cfc g R₂) ^ (1/2 : ℝ) := by
      grind [IsSelfAdjoint.of_nonneg, IsSelfAdjoint.star_mul_eq]
    have hR : IsStrictlyPositive R := by grind [isStrictlyPositive_convex_combination]
    ...
    have hT : star T₁ * T₁ + star T₂ * T₂ ≤ 1 := by
      sorry -- grind [ring, add halves 1, IsSelfAdjoint.conjugate_le_conjugate (this), IsSelfAdjoint.of_nonneg, ]
