...
`(A, B) ↦ re ⟪PowerMean α β A B x, x⟫` is jointly concave in `(A, B)`
...
theorem LiebConcavity_general' {α β : ℝ} (hα : 0 < α ∧ α ≤ 1) (hβ : 0 < β ∧ β ≤ 1) (x : H) :
    ConcaveOn ℝ {(A, B) : H × H | IsStrictlyPositive A ∧ 0 ≤ B}
      (fun (A, B) => re ⟪PowerMean α β A B x, x⟫_ℂ) := by
  refine ⟨(PowerMean_jointly_concave hα hβ).1,
        fun ⟨A₁, B₁⟩ h₁ ⟨A₂, B₂⟩ h₂ a b ha hb hab => ?_⟩
  calc a * re ⟪PowerMean α β A₁ B₁ x, x⟫_ℂ
      + b * re ⟪PowerMean α β A₂ B₂ x, x⟫_ℂ
      = re ⟪(a • PowerMean α β A₁ B₁ + b • PowerMean α β A₂ B₂) x, x⟫_ℂ := by
          simp only [ContinuousLinearMap.add_apply, ContinuousLinearMap.smul_apply,
                     inner_add_left, map_add]
          congr 1 <;>
          · rw [← Complex.coe_smul, inner_smul_left, Complex.conj_ofReal]
            simp
    _ ≤ re ⟪PowerMean α β (a • A₁ + b • A₂) (a • B₁ + b • B₂) x, x⟫_ℂ :=
          reApplyInnerSelf_mono ((PowerMean_jointly_concave hα hβ).2 h₁ h₂ ha hb hab) x

/-- With the explicit formula:
`(A, B) ↦ re ⟪A ^ (β * (1 - α)) * x * B ^ α, x⟫` is jointly concave -/
theorem LiebConcavity_general {α β : ℝ} (hα : 0 < α ∧ α ≤ 1) (hβ : 0 < β ∧ β ≤ 1) (x : H) :
    ConcaveOn ℝ {(A, B) : H × H | IsStrictlyPositive A ∧ 0 ≤ B}
...
theorem LiebConcavity {s : ℝ} (hs0 : 0 < s) (hs1 : s < 1) (x : H) :
    ConcaveOn ℝ {(A, B) : H × H | IsStrictlyPositive A ∧ 0 ≤ B}
      (fun (A, B) => re ⟪A ^ s * x * B ^ (1 - s), x⟫_ℂ) :=
  LiebExtension (by linarith) hs0 (by linarith) x

/-- ## Generalised Ando Convexity
`(A, B) ↦ re ⟪PowerMean α β A B x, x⟫` is jointly convex in `(A, B)`
...
theorem AndoConvexity_general' {α β : ℝ} (hα : 1 ≤ α ∧ α ≤ 2) (hβ : 0 < β ∧ β ≤ 1) (x : H) :
  ...
  refine ⟨(PowerMean_jointly_convex hα hβ).1,
        fun ⟨A₁, B₁⟩ h₁ ⟨A₂, B₂⟩ h₂ a b ha hb hab => ?_⟩
  calc re ⟪PowerMean α β (a • A₁ + b • A₂) (a • B₁ + b • B₂) x, x⟫_ℂ
      ≤ re ⟪(a • PowerMean α β A₁ B₁ + b • PowerMean α β A₂ B₂) x, x⟫_ℂ :=
          reApplyInnerSelf_mono ((PowerMean_jointly_convex hα hβ).2 h₁ h₂ ha hb hab) x
    _ = a * re ⟪PowerMean α β A₁ B₁ x, x⟫_ℂ
      + b * re ⟪PowerMean α β A₂ B₂ x, x⟫_ℂ := by
          simp only [ContinuousLinearMap.add_apply, ContinuousLinearMap.smul_apply,
                     inner_add_left, map_add]
          congr 1 <;>
          · rw [← Complex.coe_smul, inner_smul_left, Complex.conj_ofReal]
...
/-- With the explicit formula:
`(A, B) ↦ re ⟪A ^ (β * (1 - α)) * x * B ^ α, x⟫` is jointly convex
for `1 ≤ α ≤ 2`, `0 < β ≤ 1`, `A` strictly positive and `B` nonneg -/
theorem AndoConvexity_general {α β : ℝ} (hα : 1 ≤ α ∧ α ≤ 2) (hβ : 0 < β ∧ β ≤ 1) (x : H) :
    ConvexOn ℝ {(A, B) : H × H | IsStrictlyPositive A ∧ 0 ≤ B}
      (fun (A, B) => re ⟪A ^ (β * (1 - α)) * x * B ^ α, x⟫_ℂ) := by
